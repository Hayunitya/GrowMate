...
class PlantAdapter(
    ...
    override fun onBindViewHolder(holder: PlantViewHolder, position: Int) {
        ...
        val formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy") // <- Format baru!
        ...
        holder.binding.tvPlantAge.text = "Umur: $ageDays hari"
        ...
        val iconRes = try {
            val plantedDate = LocalDate.parse(plant.plantDate, formatter)
            val umur = ChronoUnit.DAYS.between(plantedDate, today)
            when {
                umur < 3 -> R.drawable.ic_seed
                umur < 7 -> R.drawable.ic_sprout
                umur < 14 -> R.drawable.ic_plant
